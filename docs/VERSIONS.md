...
| Reference only | Europe | English | **label conflict: Rev 5 / Rev 13** | Nintendo DS; `NTR-ADAP-EUR` reported; 2007-07-27 | SHA-1 `66d2fbfb0dbc1f86a3d726971196989b950092bc`; MD5 `ca7620b92665a2565b5c1831f5f66ec0` | The same binary hash is called **Rev 13** by OpenRetro and some technical/cheat databases, but **EUR English Rev 5** by Project Pokémon restoration/compatibility documentation and other metadata. This is `CONFLICTING_EVIDENCE`, not two established binaries. Sources `D-SRC-0003`, `D-SRC-0007`, `D-SRC-0010`, `D-SRC-0014`, `D-SRC-0022`. |
...
- The English-European SHA-1 `66d2fbfb0dbc1f86a3d726971196989b950092bc` currently has **conflicting revision labels** (`Rev 5` and `Rev 13`) across technically useful sources. Until the naming semantics are resolved, the hash is the stable identity and the revision field remains contested.
...
9. When two sources apply different revision labels to the same cryptographic hash, preserve the hash as the stable identity and record the label conflict explicitly.
